 ...
 * @author jvalsesia
 */
@WebServlet(name = "InsereTopicoServlet", urlPatterns = {"/inserirtopico"})
public class InsereTopicoServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        t.setLogin(usuarioAutenticado.getLogin());
        
        try {
            TopicoController tc = new TopicoController();
            tc.inserir(t);
            request.getSession().setAttribute("topicos", tc.listaTopicos());
            
            UsuarioController uc = new UsuarioController();
            uc.adicionarPontos(login, 10);
            request.getRequestDispatcher("topicos.jsp").forward(request, response);
        } catch (Exception ex) {
            request.setAttribute("erro", ex.getMessage());
            request.getRequestDispatcher("inseretopicos.jsp").forward(request, response);
        }
